...
This code implements Proportional Control using an analog sensor (LDR) to determine the process variable (PV). 
The control output (CO) is calculated based on the error (SP - PV) and proportional gain (pGain), controlling the LED's brightness.
It also utilizes four pushbuttons, with two of them serving as up and down buttons for adjusting the setpoint and p Gain. 
The other two pushbuttons, left and right, are used for navigating through different screens on the LCD display. 
The values of the process variable (PV), setpoint (SP), control output (CO), Error, and p Gain are displayed both on the Serial Monitor and the LCD screen.
*/
