/* Manual Control
This code implements a Manual Control system using an analog sensor (LDR) and an actuator (LED). 
The output is achieved through adjusting the input value with a potentiometer (variable resistor). 
The readings of the Input Value, Process Variable (PV), and Control Output (CO) are displayed on both the Serial Monitor and an LCD screen.
*/
